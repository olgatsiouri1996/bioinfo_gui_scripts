...
def main():
    ap = GooeyParser(description="Retrieve the secondary structure using DSSP from a pdb file by selecting the model, chain, start and end position")
    ap.add_argument("-in", "--input", required=True, widget='FileChooser', help="input pdb file(every name except: out.pdb)")
    ...
    ap.add_argument("-out", "--output", required=True, widget='FileSaver', help="output txt file with \"chain\", \"residue_number\", \"residue_name\" and \"residue_structure\" as columns(every name except: out.pdb)")
