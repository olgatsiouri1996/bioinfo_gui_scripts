...
import pandas as pd
# input parameters
@Gooey(required_cols=4, program_name='remove lines by pattern', header_bg_color= '#DCDCDC', terminal_font_color= '#DCDCDC', terminal_panel_color= '#DCDCDC')
def main():
  ap = GooeyParser(description= "remove lines that match a user specified pattern")
  ...
  ap.add_argument("-p", "--pattern", required=True, help="specify the pattern to remove the lines that have it")
