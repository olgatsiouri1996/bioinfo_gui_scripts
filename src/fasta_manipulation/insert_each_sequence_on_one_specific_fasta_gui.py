...
# input parameters
@Gooey(required_cols=4, program_name= 'insert each sequence on a specific fasta', header_bg_color= '#DCDCDC', terminal_font_color= '#DCDCDC', terminal_panel_color= '#DCDCDC')
def main():
    ap = GooeyParser(description="insert each sequence at specific positions in one specific sequence in a multi-fasta file")
    ap.add_argument("-in", "--input", required=True, widget='FileChooser',  help="input multi-fasta file")
    ap.add_argument("-is", "--insert", required=True, widget='FileChooser', help="input multi-fasta file with the sequences to insert")
