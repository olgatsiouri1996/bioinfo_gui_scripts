...
# input parameters
@Gooey(required_cols=3, program_name='spectra to protein', header_bg_color= '#DCDCDC', terminal_font_color= '#DCDCDC', terminal_panel_color= '#DCDCDC')
def main():
    ap = GooeyParser()
    ap.add_argument("-in", "--input", required=True, widget='FileChooser', help="input 1-column txt file with numbers")
    ap.add_argument("-id", "--id", required=True, type=str, help="fasta header of the output fasta file")
    ap.add_argument("-out", "--output", required=True, widget='FileSaver', help="output fasta file with the protein sequence")
